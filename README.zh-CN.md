...
通过实时感知各账号的周期剩余额度与重置时间，自动将账号聚合为梯队分流组，实现高并发平滑轮询、低长周期额度并行排空与长周期耗尽软下沉，最大化利用每一份订阅额度。
...
- **低额并行排空**：当一个重置梯队内全部账号的长周期剩余额度低于 5% 时，将该梯队并入下一健康梯队；仍有额度的账号保持等权参与，直到所有长周期额度归零。
- **短窗容错**：仅 5 小时窗口暂时偏低不会改变优先级，由 CPA 自身的可用性与冷却机制处理。
- **耗尽保底路由**：存在多个启用账号时，耗尽账号降至优先级 `0`，但仍可调用；若它是该渠道唯一启用账号，则始终保持优先级 `400`，插件绝不额外封死账号。
- **Codex 主额度路由**：Codex 调度只依据账号主额度；Spark、Code Review 等模型独立辅助额度不能在主周额度归零后继续维持普通 Codex 路由。
- **全自动自适应巡检**：额度高于 15% 每 30 分钟检查，5%–15% 每 15 分钟检查，低于 5% 每 10 分钟检查；遇 429 速率限制自动指数退避。
